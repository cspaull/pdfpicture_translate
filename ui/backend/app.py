...
class PDFImageTranslator:
    def __init__(self, source_lang='id', target_lang='vi'):
        ...
        self.reader = easyocr.Reader(ocr_langs, gpu=False)  # Set to False for CPU
    ...
    def draw_text_in_box(self, draw, text, x, y, max_width, max_height, font):
        """Draw text within box boundaries with automatic line wrapping"""
        padding = 2
        x += padding
        y += padding
        max_width -= 2 * padding
    ...
    def cluster_text(self, results, eps=60, min_samples=1):
        ...
        return [panel for panel in panels if panel]  # Remove empty panels
...
def translate_pdf():
    try:
        ...
        output_filename = f"{base_name}_translated_{target_lang}.pdf"
        output_path = os.path.join(OUTPUT_FOLDER, f"{uuid.uuid4()}_{output_filename}")
        
        # Initialize translator and process PDF
